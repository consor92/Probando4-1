...
int cancionInicio[] = {NOTE_B4,NOTE_B4,NOTE_A4,NOTE_A4,NOTE_C5,NOTE_D5};
int figuras1[] = {2,2,2,2,2,2};
int comer[] = {NOTE_B4};
int figuras2[] = {32}; 
int cancionScore[] = {NOTE_A4,NOTE_A4,NOTE_B4,NOTE_B4,NOTE_C5,NOTE_C5};
int figuras3[] = {2,2,2,2,2,2};
int cancionScore2[] = {NOTE_C5,NOTE_D5,NOTE_D5,NOTE_D5,NOTE_G4,NOTE_G4};
int figuras3_5[] = {2,2,2,2,2,2};
int cancionWin[] = {NOTE_D5 ,NOTE_D5,NOTE_C5,NOTE_A4,NOTE_A4,NOTE_A4};
int figuras4[] = {4,4,1,2,2,2}; 
int cancionLose[] = {NOTE_E4,NOTE_E4,NOTE_F4,NOTE_G4,NOTE_G4,NOTE_G4};
int figuras5[] = {4,4,1,2,2,2}; 
  ...
  if(comida.fila == -1 || comida.col == -1){
    ...
    do{
      ...
      if(comida.fila == 3 || comida.fila == 4){
        if(comida.col == 3 || comida.col == 4 || comida.col == 10 || comida.col == 11 || comida.col == 20 || comida.col == 21 || comida.col == 27 || comida.col == 28){
          comida.col += 2;
          comida.fila += 2;
        }
      }
...
void movSnake(){
  ...
  if(snake.fila == 3 || snake.fila == 4){
    if(snake.col == 3 || snake.col == 4 || snake.col == 10 || snake.col == 11 || snake.col == 20 || snake.col == 21 || snake.col == 27 || snake.col == 28){
  ...
  if(snake.fila == comida.fila && snake.col == comida.col){
    ...
    snakeVelocipy -= 3; // Aumenta la velocidad disminuyendo el tiempo en que se mueve
    comida.fila = -1;  // Resetea la comida
    comida.col = -1;
    tocarMelodia(comer,figuras2,(sizeof(comer) / sizeof(int)), buzzer);
...
void tocarMelodia(int mel[], int fig[], int tam, int pin)
{
  /*Función que recibe una melodía y figuras en arrays, el tamaño de la melodía y el pin del buzzer*/
  for(int nota = 0; nota <= tam; nota++)
  {
    int duracion = 1000 / fig[nota];
    int tempo = duracion * 1;
    tone(pin, mel[nota], duracion);
    delay(tempo);
    noTone(pin);
  }
}
...
void spawnMuros(){
  ...
  matriz.setPoint(3, 10, true);
  matriz.setPoint(3, 11, true);
  matriz.setPoint(4, 10, true);
...
void mensajeSnake(){
  ...
  mp.print("SNAKE");
  tocarMelodia(cancionInicio,figuras1,(sizeof(cancionInicio) / sizeof(int)), buzzer);
  mp.displayClear();
}
...
void mostarGameOver(){
  ...
  mp.setTextAlignment(PA_CENTER);
  mp.print("U LOSE");
  tocarMelodia(cancionLose,figuras5,(sizeof(cancionLose) / sizeof(int)), buzzer);
  mp.displayClear();
}
...
void mostrarGanaste(){
  ...
  mp.setTextAlignment(PA_CENTER);
  mp.print("U WIN");
  tocarMelodia(cancionWin,figuras4,(sizeof(cancionWin) / sizeof(int)), buzzer);
  mp.displayClear();
}
...
void mostrarScore(int score){
  ...
  mp.setTextAlignment(PA_CENTER);
  mp.print("SCORE:");
  tocarMelodia(cancionScore,figuras3,(sizeof(cancionScore) / sizeof(int)), buzzer);
  mp.print(score);
  tocarMelodia(cancionScore2,figuras3_5,(sizeof(cancionScore2) / sizeof(int)), buzzer);
  mp.displayClear();
}
